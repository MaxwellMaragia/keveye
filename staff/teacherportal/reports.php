...
function teachersComment($grade){
    ...
    $where_grade=array('period'=>$year_term,'class'=>$class);
    $fetch_final = $obj->fetch_records('final_result',$where_grade);
    ...
    if($fetch_final)
    {
        foreach($fetch_final as $row)
        {
            ...
            $where_student = array('AdmissionNumber'=>$admission);
            $fetch_students = $obj->fetch_records('student',$where_student);
...
$term = $year_term[10];
$pathdir= "reports/";
$nameArchive = "Form-$class-$year-term-$term-reports.zip";
$zip = new ZipArchive();
