...
$fetch_results = $obj->fetch_records('cycle_one',$where);
if($fetch_results)
{
    foreach($fetch_results as $row)
    {
        ...
        //get ranking
        //get total students
        $sql_class="SELECT * FROM cycle_one WHERE class='$class' AND period='$year_term'";
        $sql_form="SELECT * FROM cycle_one WHERE form='$class[0]' AND period='$year_term'";
        ...
        //get class rank
        $query = "SELECT admission, average FROM cycle_one WHERE class='$class' AND period='$year_term' ORDER BY average DESC ";
...
//get form rank
        $sql = "SELECT admission, average FROM cycle_one WHERE form='$class[0]' AND period='$year_term' ORDER BY average DESC ";
        $execute = mysqli_query($obj->con,$sql);
...
$term = $year_term[10];
$pathdir= "reports/";
$nameArchive = "Form-$class-$year-term-$term-result-slips-cycle-one.zip";
$zip = new ZipArchive();
