...
$where=array("admission"=>$admission[$i],"period"=>$period,"subject"=>"CRE");
$get_cre=$obj->fetch_records("results",$where);

//if chemistry results exist
if($get_cre)
{

    foreach($get_cre as $row) {
    ...
    if($mark[$i]>$cre[$i])
    {
        ...
        $total_new[$i]=$total_mark[$i]+$mark[$i]-$cre[$i];
    ...
    else
    {
        ...
        $new_average[$i]=($total_mark[$i]-$mark[$i])/$min;
        echo $new_average[$i];


        //fetching the new grade using average
        $sql_grade="SELECT * FROM grading_system WHERE upper_limit>=round($new_average[$i]) AND lower_limit<=round($new_average[$i])";
        $execute_grade=mysqli_query($obj->con,$sql_grade);
